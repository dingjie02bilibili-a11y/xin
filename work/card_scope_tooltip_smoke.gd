...
func run_test() -> void:
	...
	for upgrade in game.UPGRADES:
		...
		if game.is_training_card(id):
			assert(game.skill_description(id).contains("买下马上用掉，不占卡位"), "Training card is not clearly disposable: " + id)
		else:
			assert(game.skill_description(id).contains("怎么放：【"), "Card is missing an effect scope: " + id)
	...
	assert(tooltip_events[0].contains("怎么放：【放在宠物左边才生效】"), "Tooltip does not expose the card scope")
